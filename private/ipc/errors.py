...
class IPCContractError(Exception):
    """Sentinel exception type for IPC contract errors, enabling isinstance() checks."""
    pass


# Named sentinel constants — one per contract error (mirrors Go package-level var declarations)
...
# Map from 4-byte selector hash to sentinel instance
...
def ignore_offset_error(error: Exception) -> Optional[Exception]:
    ...
    if mapped_error is ErrOffsetOutOfBounds:
        return None
    return error
